...
class PaintingApplication extends StatelessWidget {
  const PaintingApplication({super.key});
  ...
  Widget _buildImageInWave() {
    return ClipRRect(
      ...
      child: Image.asset(
        'images/625648.jpg', // Imagen para esta pantalla
        height: 280,
        fit: BoxFit.cover,
      ),
    );
  ...
  Widget _buildButtons(BuildContext context, Size size) {
    final services = [
      {"text": "MUROS", "icon": Icons.wallpaper, "route": "/painting_walls"},
      {
        "text": "FIRMES (LAY/OUT)",
        "icon": Icons.layers,
        "route": "/painting_walls"
      },
      {
        "text": "ESTRUCTURAS METÁLICAS",
        "icon": Icons.build,
        "route": "/painting_metal_structures"
      },
    ];
